...
class pancreas(data.Dataset):
    def __init__(self, mode, data_path, current_fold, organ_number, low_range, high_range, slice_threshold, slice_thickness, \
        ...
        image_list_file = os.path.join(list_path, 'training_' + 'FD' + str(current_fold) + '.txt')
        image_list = open(image_list_file, 'r').read().splitlines()
        ...
        slice_list_trainining = os.path.join(list_path, 'training_' + plane + '.txt')
        slice_list = open(slice_list_trainining, 'r').read().splitlines()
        ...
        for l in range(self.slices):
            ...
            ss = s[3].split('labels')
            sss = ss[1].split('.npy')
            self.prob_filename[l] = ss[0] + 'probs' + sss[0] + '.npy'
    ...
    def load_data(self):
        if self.slice_thickness == 1:
            ...
            label[0, ...] = np.load(self.label_filename[self.index1])
            label[1, ...] = np.load(self.label_filename[self.index1])
            label[2, ...] = np.load(self.label_filename[self.index1])
            prob = np.zeros((3, width, height), dtype=np.uint8)
            prob[0, ...] = np.load(self.prob_filename[self.index1])
            prob[1, ...] = np.load(self.prob_filename[self.index1])
            prob[2, ...] = np.load(self.prob_filename[self.index1])
        ...
        label = (label == self.organ_ID).astype(np.uint8)
