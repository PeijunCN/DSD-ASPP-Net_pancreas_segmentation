...
class DenseASPP(nn.Module):
    ...
    * output_scale can only set as 8 or 16
    ...
    def forward(self, _input, _saliency):
        ...
        ###### geo_dist_map###################
        crop_image1 = _saliency[0, :, :, :].numpy().transpose(1, 2, 0)
        plt.imshow(crop_image1, cmap='hot')
        plt.show()
        plt.axis('off')
        #######################################
        saliency =self.downsample(self.downsample(_saliency))# 1/4 downsample
        ###### saliency_map###################
        temp = self.value_conv1(saliency.cuda())
        crop_image2 = temp[0, 0, :, :].cpu().detach().numpy()
        plt.imshow(crop_image2, cmap='gray')
        plt.show()
        plt.axis('off')
        #######################################
        ###### saliency-aware feature_map###################
        temp3 = feature*self.value_conv1(saliency.cuda())
        # for i in range(384): #5
        #     if feature[0, i, :, :].sum().item() >0:
        #         crop_image3 = temp3[0, i, :, :].cpu().detach().numpy()
        #         plt.imshow(crop_image3, cmap='gray')
        #         plt.show()
        #         plt.axis('off')
        crop_image3 = temp3[0, 5, :, :].cpu().detach().numpy()
        plt.imshow(crop_image3, cmap='gray')
        plt.show()
        plt.axis('off')
        #######################################
        ###### saliency-aware feature_fusion_map###################
        feature = feature + self.gamma*feature*self.value_conv1(saliency.cuda())
        crop_image4 = feature[0, 5, :, :].cpu().detach().numpy()
        plt.imshow(crop_image4, cmap='gray')
        plt.show()
        plt.axis('off')
        ...
        ###### saliency_map###################
        temp = self.value_conv2(saliency.cuda())
        crop_image2 = temp[0, 0, :, :].cpu().detach().numpy()
        plt.imshow(crop_image2, cmap='gray')
        plt.show()
        plt.axis('off')
        #######################################
        ###### saliency-aware feature_map###################
        temp3 = feature * self.value_conv2(saliency.cuda())
        for i in range(384): #5
            if feature[0, i, :, :].sum().item() >0:
                crop_image3 = feature[0, i, :, :].cpu().detach().numpy()
                plt.imshow(crop_image3, cmap='gray')
                plt.show()
                plt.axis('off')
        # crop_image3 = temp3[0, 5, :, :].cpu().detach().numpy()
...
class SADDenseNet(nn.Module):
    ...
    def forward(self, image, label=None, score=None, mode='coarse'):
        ...
        if self.TEST is None:
            ...
            crop_image2 = cropped_score[0, 1, :, :].cpu().detach().numpy()
            plt.imshow(crop_image2, cmap='gray')
            plt.show()
            plt.axis('off')
