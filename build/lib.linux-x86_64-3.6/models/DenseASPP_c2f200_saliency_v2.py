...
class SADDenseNet(nn.Module):
    ...
    def forward(self, image, label=None, score=None, mode='coarse'):
        ...
        if self.TEST is None:
            if mode =='coarse':
                ...
                if score.sum().item() > 0:
                    d = self.geodist(score.cpu(), image.cpu())
                ...
                cropped_image, crop_info, cropped_label = self.croptrainfine(label, image.cpu())
                h = cropped_image.cuda()
                h = self.model(h)
                del cropped_image, d
            ...
            h = torch.zeros_like(image)
            if score.sum().item() > 0:
                h = self.geodist(score.cpu(), image.cpu())
            h = self.relu_saliency1(self.saliency1(h.cuda()))
            h = self.saliency2(h)
            image = image.cuda() * h

            cropped_image, crop_info, cropped_label = self.croptrainfine(label, image.cpu())
            h = self.model(cropped_image.cuda())
            h = F.sigmoid(h)
    ...
    def croptrainfine(self, prob_map, saliency_data, score=None):
        ...
        if (bbox[1]-bbox[0]) < 96:
            bbox[0] = int(bbox[0] - (96-(bbox[1]-bbox[0]))/2)
            bbox[1] = 96 + bbox[0]

        if (bbox[3]-bbox[2]) < 96:
            bbox[2] = int(bbox[2] - (96-(bbox[3]-bbox[2]))/2)
            bbox[3] = 96 + bbox[2]
        ...
        cropped_image_rs = transform.resize(cropped_image.detach().numpy(), (N, C, 200, 200))
        cropped_image = torch.from_numpy(cropped_image_rs)

        cropped_label_rs = transform.resize(cropped_label.detach().numpy(), (N, C, 200, 200))
        ...
        return cropped_image, crop_info, cropped_label
