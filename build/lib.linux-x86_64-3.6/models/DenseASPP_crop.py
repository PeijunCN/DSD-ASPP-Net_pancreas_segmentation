...
class DenseASPP(nn.Module):
    ...
    * output_scale can only set as 8 or 16
    ...
    def forward(self, _input, label=None):
        image = _input
        cropped_image, crop_info = self.crop_coarse(image)

        feature = self.features(cropped_image)
        ...
        cls = self.uncrop(crop_info, cls, image)
    ...
    def uncrop(self, crop_info, cropped_image, image):
        (N, C, W, H) = image.shape
        uncropped_image = torch.ones([N, self.n_class, H, W]).cuda()
