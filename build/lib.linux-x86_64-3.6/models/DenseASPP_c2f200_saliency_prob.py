...
class SADDenseNet(nn.Module):
        ...
        if self.TEST is None:
            if mode =='coarse':
                ...
                if cropped_score.sum().item() > 0:
                    crop_image1 = cropped_score[0].detach().cpu().numpy().transpose(1, 2, 0)
                    crop_image2 = cropped_image[0].detach().cpu().numpy().transpose(1, 2, 0)
                    crop_image3 = cropped_label[0].detach().cpu().numpy().transpose(1, 2, 0)
            ...
            h = cropped_score
            h1 = self.relu_saliency1(self.saliency1(h.cuda()))
            h2 = self.saliency2(h1)
            h = cropped_image.cuda() * h2
    ...
    def croptrainfine(self, prob_map, saliency_data, score=None):
        ...
        if (bbox[1]-bbox[0]) < 80:
            bbox[0] = max(int(bbox[0] - (80-(bbox[1]-bbox[0]))/2), 0)
            bbox[1] = 80 + bbox[0]

        if (bbox[3]-bbox[2]) < 80:
            bbox[2] = max(int(bbox[2] - (80-(bbox[3]-bbox[2]))/2), 0)
            bbox[3] = 80 + bbox[2]
