...
class SADDenseNet(nn.Module):
    ...
    def forward(self, image, label=None, score=None, mode='coarse'):
        if self.TEST is None:
            if mode =='coarse':
                ...
                cropped_image, crop_info, cropped_label = self.croptrainfine(label, image)
                h = self.geodist(cropped_label.cpu(), cropped_image.cpu())
                h = self.relu_saliency1(self.saliency1(h.cuda()))
                h = self.saliency2(h)
                h = cropped_image.cuda()*h
            ...
            h = self.geodist(label, image)
            h = self.relu_saliency1(self.saliency1(h))
            h = self.saliency2(h)
            saliency = h

            h, crop_info = self.crop(label, image*saliency)
            h = self.model(h)
            h = self.uncrop(crop_info, h, image)
            return h
    ...
    def geodist(self, prob, image):
        ...
        for i in range(N):
            for j in range(C):
                ...
                img_ = image[i, 1].reshape([W, H]).cpu().numpy()
    ...
    def croptrainfine(self, prob_map, saliency_data):
        (N, C, W, H) = prob_map.shape
        ...
        cropped_label_rs = transform.resize(cropped_label.numpy(), (N, C, 224, 224))
