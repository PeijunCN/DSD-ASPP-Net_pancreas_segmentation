...
class DenseASPP(nn.Module):
    ...
    * output_scale can only set as 8 or 16
    ...
    def __init__(self, model_cfg, n_class=2, output_stride=8):
        ...
        self.saliency3 = nn.Conv2d(n_class, 3, kernel_size=5, stride=1, padding=2)
    ...
    def forward(self, _input, _prob):
        batchsizenum, channel, row, col = _input.shape

        prob = _prob[:, 1].reshape(batchsizenum, 1, row, col)
        #print(prob.size())
        temp = 1-prob
        #print(temp.shape)
        prob =torch.cat((temp, prob), 1)

        h = self.relu_saliency1(self.saliency1(prob))
        saliency = self.saliency3(h)
