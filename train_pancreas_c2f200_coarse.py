...
from datasets import pancreas_nii_coarse
...
ckpt_path = 'ckpt'
exp_name = 'modelCoarseC2f200_3_6_18'
...
def main(train_args):
    ...
    #model_path = './ckpt/model_fine_c2frs/FD0:Z3_1_14.pth'
    is_local = False
    curr_epoch = 1
    ...
    if is_local:
        ...
        for k, v in weight.items():
            if k !='module.classification.1.weight'and k!='module.classification.1.bias':
                ...
                ss = name.split('.')
                if ss[1] != 'ASPP_24' and ss[1] != 'ASPP_12' and ss[1]!= 'ASPP_18':
    ...
    train_joint_transform = joint_transforms.Compose([joint_transforms.RandomRotate(10)])
    ...
    train_set = pancreas_nii_coarse.pancreas('train', data_path=data_path, current_fold=int(current_fold), organ_number=organ_number, \
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=2, shuffle=True, num_workers=16, drop_last=True)
...
def train(train_loader, net, criterion, optimizer, epoch, train_args):
    ...
    for i, (input, label) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output = net(input, label, mode='coarse')
