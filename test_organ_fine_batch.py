...
from models.DenseASPP_c2f200_saliency_module3_new import SADDenseNet
...
for t in range(len(snapshot)):
    ...
    for i in range(len(volume_list)):
        ...
        if not os.path.isfile(volume_file):
            ...
            break
    if not finished:
        net = SADDenseNet(Model_CFG, n_class=N_CLASS, output_stride=8, TEST=TEST)
    ...
    for i in range(len(volume_list)):
        ...
        ss = s[2].split('labels')
        sss = ss[1].split('.npy')
        scorename = ss[0] + 'probs' + sss[0] + '.npy'
        score = np.load(scorename)
        score = (score >= 128).astype(float)
        ...
        if not os.path.isfile(volume_file):
            ...
            for j in range(minR, maxR):
                ...
                if plane == 'X':
                    image_ = image[sID, :, :].astype(np.float32).transpose(1, 2, 0)
                    score_ = score[sID, :, :].astype(np.float32).transpose(1, 2, 0)
                elif plane == 'Y':
                    image_ = image[:, sID, :].transpose(0, 2, 1).astype(np.float32)
                    score_ = score[:, sID, :].transpose(0, 2, 1).astype(np.float32)
                elif plane == 'Z':
                ...
                test_img = Variable(data_transforms(test_img).unsqueeze(0), volatile=True)
                score_ = score_.transpose(2, 0, 1)
                score_ = torch.FloatTensor(score_)
                score_ = Variable(score_.unsqueeze(0), volatile=True)

            pre = net.forward(test_img, score=score_)
            # pre = F.sigmoid(pre)
